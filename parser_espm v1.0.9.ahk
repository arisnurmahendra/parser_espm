...
; ESPMWindow.OnEvent("Close", (*) => ESPMWindow := "")

; ESPMWindow.Navigate("https://espm.bpjt.pu.go.id/v1-satemuan")
; ESPMWindow.ExecuteScript("(function () {if (document.getElementById('ahk-text-mining')) return;const btn = document.createElement('button');btn.id = 'ahk-text-mining';btn.textContent = '🧠 Text Mining';Object.assign(btn.style, {position: 'fixed',top: '10px',right: '10px',zIndex: 999999,padding: '10px 16px',background: '#0d6efd',color: '#fff',border: 'none',borderRadius: '6px',cursor: 'pointer'});btn.onclick = () => {chrome.webview.postMessage({cmd: 'text_mining',html: document.documentElement.outerHTML});};document.body.appendChild(btn);})();")
; -----------------------------------------------------------
...
; MsgBox("configSys = " . configSys)
; MsgBox("configSys[`"Debug`"] = " . configSys["Debug"] . "`nstatus:" . (configSys["Debug"] == 1))
...
WebViewMessageHandler(WebView, Message) {
    ...
    try {
        msg := JSON.parse(Message, false, true)
...
WebJsonEvent(WebView, Message) {
    global configSys, MyWindow, pendingDownloadData
    ...
    try {
        ...
        manifestText := obj.Get("manifestText", "")

        if (action = "ping") {
            return JSON.stringify(Map("ok", true, "action", "ping"))
        }

        LogDebug("WebJsonEvent action=" action)

        if (action = "config") {
            ; Simpan konfigurasi baru
            SaveJsonToDisk(data, "config.json")
            LoadConfigFromFile()
            return '{"ok":true,"action":"config"}'

        } else if (action = "download") {
            ; Simpan manifest dan mulai unduhan gambar
            LogDebug("download: received")
            try FileDelete "manifestPath.json"
            if !manifestText {
                throw Error("manifestText kosong dari JavaScript")
            }
            pendingDownloadData := ParseDownloadManifestText(manifestText)
            LogDebug("download: data parsed type=" Type(pendingDownloadData) " length=" pendingDownloadData.Length)
            if !manifestJson {
                manifestJson := JSON.stringify(pendingDownloadData, true)
            }
            LogDebug("download: manifest length=" StrLen(manifestJson))
            FileAppend manifestJson, "manifestPath.json", "UTF-8"
            LogDebug("download: manifest written")
            SetTimer(() => DownloadImages(pendingDownloadData), -10)
            LogDebug("download: worker scheduled")
            return '{"ok":true,"action":"download","message":"Download started"}'
            ...
            return '{"ok":true,"action":"request_config"}'
        }

        return JSON.stringify(Map("ok", false, "error", "Unknown action"))

    } catch Error as e {
        LogDebug("WebJsonEvent error: " e.Message)
        return JSON.stringify(Map("ok", false, "error", e.Message))
    }
}
...
LoadConfigFromFile(fileName := "config.json") {
    global configSys, default_config, MyWindow
    ...
    try {
        ...
        parsed := JSON.parse(jsonStr, false, true)
...
ParseDownloadManifestText(manifestText) {
    dataArray := []
    ...
    Loop Parse manifestText, "`n", "`r" {
        ...
        if (parts.Length < 5) {
            throw Error("Baris manifestText tidak valid pada baris " A_Index)
        }
    ...
    if (dataArray.Length = 0) {
        throw Error("manifestText tidak berisi data gambar")
    }
...
DownloadImages(dataArray) {
    global configSys, MyWindow  ; gunakan konfigurasi global
    ...
    try {
        ...
        Loop dataArray.Length {
            try {
                ...
                failedNo := IsSet(no) ? no : A_Index
                failedName := IsSet(imgName) ? imgName : "Item " A_Index
                try MyWindow.ExecuteScriptAsync(Format("renderCardsWithDelay('failed', {}, {}, {}, {}, {})"
                    , A_Index
                    , dataArray.Length
                    , failedNo
                    , JSEscape(failedName)
                    , JSEscape(itemError.Message)
                ))
                MsgBox "Gagal proses item " A_Index ":`n" itemError.Message
            }
        }
        ...
        MsgBox "DownloadImages error:`n" e.Message
        return false
    }
}
...
CompressAndOverlayImage(imgPath, tanggal, latlong, imgName) {
    ...
    ; Load gambar asli
    LogDebug("overlay: start file=" imgPath)
    pBitmap := Gdip_CreateBitmapFromFile(imgPath)
    LogDebug("overlay: bitmap loaded value=" pBitmap)
    if (!pBitmap || pBitmap < 0) {
        Gdip_Shutdown(pToken)
        throw Error("Gagal memuat gambar: " imgPath " Kode GDI+: " pBitmap)
    }
    orientation := GetImageOrientation(imgPath)
    LogDebug("overlay: orientation=" orientation)
    ...
    LogDebug("overlay: save to=" outPath)
    ; Hapus file jika sudah ada
    if FileExist(outPath)
        FileDelete outPath
    ; Simpan gambar
    if (SubStr(outPath, -3) = ".jpg" || SubStr(outPath, -4) = ".jpeg") {
        saveStatus := Gdip_SaveBitmapToFile(pBitmap, outPath, 100)
    } else {
        saveStatus := Gdip_SaveBitmapToFile(pBitmap, outPath)
    }
    if (saveStatus != 0) {
        Gdip_DisposeImage(pBitmap)
        Gdip_Shutdown(pToken)
        throw Error("Gagal simpan gambar export. Kode GDI+: " saveStatus " Path: " outPath)
    }
    Gdip_DisposeImage(pBitmap)
    Gdip_Shutdown(pToken)
    return outPath
...
LoadJson(filePath) {
    ...
    return JSON.parse(jsonText, false, true)
}

LogDebug(message) {
    try FileAppend FormatTime(A_Now, "yyyy-MM-dd HH:mm:ss") " " message "`n", A_ScriptDir "\ahk_debug.log", "UTF-8"
}
